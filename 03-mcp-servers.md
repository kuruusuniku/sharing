...
### たとえ話：MCP は AI の USB-C

スマートフォンの充電器を思い出してください。昔は、メーカーごとに充電端子が違いました。Micro-USB、Lightning、独自規格...。ケーブルの引き出しがカオスになった経験、ありませんか？

USB-C が登場して、すべてが1つのコネクタに統一されました。充電も、データ転送も、映像出力も、全部同じ端子。

**MCP は AI における USB-C です。** 1つの標準プロトコルで、JIRA でも GitHub でも Slack でもカレンダーでも、なんでも繋がる。ツールごとにカスタム連携を書く時代は終わりました。

### MCP 以前の世界

MCP が存在しなかったころ、AIに外部ツールを使わせるには、**ツールごとに専用のコードを書く必要がありました**。

- GitHub 連携を作りたい？ → GitHub API を叩くコードを書いて、認証を管理して、レスポンスを整形して Claude に渡す
- JIRA も欲しい？ → 同じことを JIRA API 向けにもう一回
- Slack も？ → もう一回...

これが10ツールになったら、10個の連携コードを保守しなければならない。しかも、各 AI ツール（Claude Code、Cursor、Windsurf...）ごとに連携の実装が違う。

MCP はこの問題を「プロトコルの標準化」で解決しました。一度 MCP サーバーを書けば、どの AI ツールからでも同じ方法で使える。コミュニティが作った MCP サーバーをそのまま使える。車輪の再発明が不要になったのです。
...
### Q: セキュリティは大丈夫？ 社内データを外部に送ってしまわない？

**A:** MCP サーバーはあなたのローカルマシン（または社内サーバー）上で動作します。Claude が MCP 経由でツールを呼ぶとき、データの流れは「Claude → あなたのPC上の MCP サーバー → 外部サービス」です。MCP サーバー自体がどのデータを取得し、どこに送るかは設定とコードで制御できます。ただし、会話の中に取り込まれたデータは Claude のコンテキストに入るため、Anthropic のサーバーには送信されます。機密度の高い情報を扱う場合は、Self-hosted 版の Recall や、ローカルLLM との組み合わせを検討してください。

### Q: 自作MCPサーバーは難しい？

**A:** 意外と簡単です。MCP サーバーは本質的に「stdin/stdout で JSON をやり取りするプログラム」です。TypeScript で50行程度から書けます。Anthropic が SDK（`@modelcontextprotocol/sdk`）を提供しているので、ボイラープレートは最小限です。社内の独自APIに繋ぎたい場合は、自作する価値が十分にあります。

### Q: どの MCP から始めるべき？

**A:** あなたが一番よく使うツールから始めてください。多くの開発者にとっては GitHub MCP が最初の一歩になります。Issue の確認、PR の作成が会話の中で完結するのは想像以上に快適です。次に Slack（チームコミュニケーション）、その次にタスク管理ツール（JIRA / Linear）という順がおすすめです。
